...
import { ObjectUsersService } from "./services/objectusers.service";
import GeneralService, { TesterFunctions } from "../../../potentialQA_SDK/server_side/general.service";

export async function ObjectUsers(generalService: GeneralService, addonService: GeneralService, request, tester: TesterFunctions) {
    //setting 'mocha verbs' to use
    const describe = tester.describe;
    const expect = tester.expect;
    const it = tester.it;

    describe('ObjectUsers Suites', () => {//the string inside the desribes will effect the report - name should be changed
        ...
        let userExternalID;
        let userEmail;
        describe('ObjectUsers Example Suit', () => {
            const objectUsersService = new ObjectUsersService(addonService);
            it('Get initial user quantity and verify user object', async () => {
                initialUsersList = await objectUsersService.getUsers();
            ...
            it('Verify GET optional fields', async () => {
                const optionalUsersFields = await objectUsersService.getUsers({
                    fields: [
                        'Name',
                        'EmployeeType',
                        'IsSupportAdminUser',
            ...
            it('Create User', async () => {
                ...
                userEmail =
                    ...
                    Math.floor(Math.random() * 1000000).toString() +
                    '.com';
                createdUser = await objectUsersService.createUser({
                ...
                const repProfile = await objectUsersService.getRepProfile();
                const securityGroups = await objectUsersService.getSecurityGroup(generalService.getClientData('IdpURL'));
                ...
                const getCreatedUserOptional = await objectUsersService.getUsers({
                ...
                const getCreatedUser = await objectUsersService.getUsers({ where: `InternalID='${createdUser.InternalID}'` });
                ...
                const newQuantity = (await objectUsersService.getUsers()).length;
                expect(newQuantity == currentUserQuantity + 1);
            });
            it('Update User', async () => {
                updatedUser = await objectUsersService.updateUser({
                    ExternalID: userExternalID,
                    Email: userEmail,
                    FirstName: Math.random().toString(36).substring(7),
                    LastName: Math.random().toString(36).substring(7),
                ...
                const getUpdatedUser = await objectUsersService.getUsers({ where: `InternalID='${updatedUser.InternalID}'` });
